...
**Albuquerque limits are not yet approved for suppression.** Its city camera list establishes locations and approaches, but not a complete current set of posted limits. The city’s public SpeedLimits ArcGIS endpoints returned empty layer lists. A subsequent [data investigation](DATA-RESEARCH.md) found a working NMDOT SpeedLimit layer, with nearby road-segment candidates for all 20 city approaches currently represented as points. Old edit dates, unit confirmation and matching the correct road/direction still require review. OSM numeric maxspeed tags and historical traffic studies are candidates, not automatic approval. Albuquerque sirens therefore remain enabled when driving below an unverified limit.
