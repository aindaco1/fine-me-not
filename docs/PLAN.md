...
- Published mobile-camera sites and deployment corridors are included as “Possible speed camera.” This describes an enforcement location, not confirmed equipment presence. Use the same brief siren; the qualification appears in the warning text.
...
Before the private trial, reconcile every alert site on the agreed routes; before claiming metro coverage, reconcile the full current inventories and audit the remaining metro jurisdictions. Establish fixed device/monitored-lane coordinates or mobile-site/corridor geometry, source-backed direction, status, and duplicate groups. Do not substitute an intersection's geocoded center for the camera. Maintain unresolved records in the same schema with an explicit review state, outside the alert snapshot.
...
| **Uncertain presence is different from uncertain location** | “Possible speed camera” is valid for a documented mobile deployment site/corridor with usable geometry. It does not make an unknown coordinate, unidentified CCTV camera, or contradictory record eligible. Pending/test sites stay out; warning-stage installations may be included when their existence and eligibility are established. |
...
The first implementation should have one app-lifetime owner of a `CLServiceSession` requiring Always authorization, a retained `CLBackgroundActivitySession`, and a `CLLocationUpdate.liveUpdates` stream configured for automotive use. Start the feature and permissions flow in the foreground. Persist the user’s enabled preference, and restore the session and update stream immediately on a permitted background relaunch. Do not tie monitoring to the settings view’s lifecycle. These are documented session patterns, but their suitability for this persistent feature still needs device validation. [Apple background-location guidance](https://developer.apple.com/documentation/corelocation/handling-location-updates-in-the-background), [Apple session lifecycle explanation](https://developer.apple.com/videos/play/wwdc2024/10212/)
...
- Do not infer operating hours or a speed limit when absent. Exclude removed, pending, test, and unresolved records. Eligible mobile sites/corridors use “Possible speed camera” and the same siren; do not label them as fixed or currently occupied.
