...
All 40 current city-listed approaches are represented: 19 matched camera points and 21 approximate warning areas. The supplement also includes Golf Course (both directions), Bridge WB in Bernalillo County, and all three published Rio Rancho NM 528 mobile corridors. Other county approaches and Rio Rancho local-road deployments remain in review; exact device coordinates for the city warning areas remain unverified. `Data/Review/publisher-report.json` provides the exact current counts and unresolved candidates. Do not describe the beta as complete Albuquerque metro coverage. Generic OSM records outside the metro are not individually corroborated against government sources.
